:root{--o9:#1A2E1A;--o8:#263D26;--o7:#3A5C3A;--o6:#4B7A4B;--o5:#5F9A5F;--o4:#7FB97F;--o3:#A8D4A8;--o2:#CBE8CB;--o1:#E4F3E4;--o0:#F3FAF3;--g5:#E6A817;--g4:#F4C430;--g3:#FFE082;--g2:#FFF3D0;--cr:#FDFCF8;--wm:#FAF6EF;--tx:#1A2E1A;--t2:#4A6A4A;--tm:#8AA08A;--rd:#C62828}
...
.pcard{background:#fff;border:1.5px solid #EEE;border-radius:10px;overflow:hidden;transition:all .15s;display:flex;flex-direction:column}
...
.pimg-w{width:100%;aspect-ratio:1;display:flex;align-items:center;justify-content:center;overflow:hidden;position:relative;background:var(--wm)}
.pimg{width:88%;height:88%;object-fit:contain;transition:transform .3s}
.pcard:hover .pimg{transform:scale(1.06)}
.pimg-fb{width:100%;height:100%;display:flex;align-items:center;justify-content:center;font-size:32px;opacity:.5}
...
/* Auth screen */
.auth{min-height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:24px;background:linear-gradient(160deg,var(--o9) 0%,var(--o8) 50%,#2A4A2A 100%)}
...
.auth-logo-m{width:48px;height:48px;background:var(--g5);border-radius:50%;display:inline-flex;align-items:center;justify-content:center;font-size:20px;font-weight:700;color:var(--o9);font-family:'Playfair Display',serif;margin-bottom:8px}
.auth-logo-t{font-family:'Playfair Display',serif;font-size:22px;color:var(--o9)}
.auth-logo-s{font-size:11px;color:var(--tm);margin-top:2px}
.auth-tabs{display:flex;gap:2px;background:#F0F0F0;border-radius:8px;padding:2px;margin-bottom:18px}
.auth-tab{flex:1;padding:8px;text-align:center;font-size:12px;font-weight:500;border:none;background:0;border-radius:6px;cursor:pointer;font-family:'DM Sans',sans-serif;color:var(--t2)}
.auth-tab.on{background:#fff;color:var(--o8);font-weight:600;box-shadow:0 1px 3px rgba(0,0,0,.06)}
.auth-err{background:var(--rd);color:#fff;padding:8px 12px;border-radius:8px;font-size:11px;margin-bottom:12px;text-align:center}
.auth-or{text-align:center;font-size:10px;color:var(--tm);margin:14px 0;position:relative}
.auth-or::before,.auth-or::after{content:'';position:absolute;top:50%;width:35%;height:1px;background:#E0E0E0}
.auth-or::before{left:0}
.auth-or::after{right:0}
.auth-google{width:100%;padding:10px;border:1.5px solid #DDD;border-radius:8px;background:#fff;cursor:pointer;font-size:12px;font-weight:500;font-family:'DM Sans',sans-serif;color:var(--tx);display:flex;align-items:center;justify-content:center;gap:8px;transition:.2s}
.auth-google:hover{border-color:var(--o4);background:var(--o0)}
...
/* Logout */
.logout-btn{margin-top:16px;width:100%;padding:10px;border:1.5px solid #DDD;border-radius:8px;background:#fff;cursor:pointer;font-size:12px;color:var(--rd);font-weight:600;font-family:'DM Sans',sans-serif;transition:.2s}
.logout-btn:hover{border-color:var(--rd);background:var(--rd);color:#fff}

/* Contact and action buttons */
.contact-btn{display:inline-flex;align-items:center;gap:4px;padding:5px 10px;border:1px solid #DDD;border-radius:8px;background:#fff;color:var(--t2);font-size:10px;font-weight:500;cursor:pointer;transition:.15s;text-decoration:none;font-family:'DM Sans',sans-serif}
.contact-btn:hover{border-color:var(--b4);background:var(--b0)}
.add-prod-btn{width:100%;padding:8px;border:1.5px dashed var(--b3);border-radius:8px;background:0;color:var(--b6);font-size:11px;font-weight:600;cursor:pointer;transition:.2s;margin-top:8px;font-family:'DM Sans',sans-serif}
